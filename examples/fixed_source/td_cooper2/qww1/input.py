...
# Set materials
m = mcdc.material(
    capture=np.array([0.2]), scatter=np.array([[0.8]]), speed=np.array([[v]])
)
m_barrier = mcdc.material(
    capture=np.array([1.0]), scatter=np.array([[4.0]]), speed=np.array([[v]])
)
...
mcdc.source(
    x=[0.0, 20.0],
    y=[0.0, 20.0],
    prob=20.0 * 20.0 / 1.0e1,
    weight=pi * 4.0e-10 * 1.0e1,
    isotropic=True,
)

# source
mcdc.source(
    time=[0.0, 10.0], x=[0.0, 5.0], y=[0.0, 5.0], prob=10.0 * v * 5 * 5, isotropic=True
)
...
mcdc.tally(
    scores=["flux", "flux-x", "flux-y", "flux-t", "n", "n-x", "n-y", "n-t"],
...
# Setting
mcdc.setting(N_particle=2e6, time_boundary=10.0 + 2e-5, bank_max=2e6, rng_seed=12345)
