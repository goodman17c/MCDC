...
for Np in Nplist:
    for j in range(1, 3):
        ...
        mcdc.setting(
            ...
            output="8_" + str(2**j) + "_" + str(Nt) + "_" + str(Np),
        )

        phi_ref = np.zeros([Nt, 21])
        phi_ref[0, 10] = 1
        mcdc.weight_window(
            x=np.linspace(-21, 21, 22),
            t=t,
        ...
        mcdc.auto_weight_window(method="semi-implicit loqd")
