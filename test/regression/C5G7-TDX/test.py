...
def test():
    ...
    assembly_1 = mcdc.cell(
        [+x0, -x1, +y2, -y3, +z1, -z2], lattice_1, lattice_center=center
    )

    center += np.array([pitch * 17, 0.0, 0.0])
    assembly_2 = mcdc.cell(
        [+x1, -x2, +y2, -y3, +z1, -z2], lattice_2, lattice_center=center
    )

    center += np.array([-pitch * 17, -pitch * 17, 0.0])
    assembly_3 = mcdc.cell(
        [+x0, -x1, +y1, -y2, +z1, -z2], lattice_3, lattice_center=center
    )

    center += np.array([pitch * 17, 0.0, 0.0])
    assembly_4 = mcdc.cell(
        [+x1, -x2, +y1, -y2, +z1, -z2], lattice_4, lattice_center=center
    )
    ...
    z_grid = np.linspace(
        -(core_height / 2 + refl_thick), (core_height / 2 + refl_thick), 2
    )
